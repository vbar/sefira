...
{
    int res;
    if (compareimpl::have_nulls(m, n, res)) {
	return res;
    }
...
{
    if (!a) {
        throw std::invalid_argument("can't compare empty namespace");
    }

    if (!(a->href)) {
        throw std::invalid_argument("can't compare unnamed namespace");
    }

    if (!b) {
        throw std::invalid_argument("cannot compare empty namespace");
    }

    if (!(b->href)) {
        throw std::invalid_argument("cannot compare unnamed namespace");
    }
...
int compareimpl::compare_attr(xmlAttrPtr a, xmlAttrPtr b)
{
    ...
    xmlChar *v = xutil::get_value(a);
    xmlChar *w = xutil::get_value(b);
...
{
    if (!p->name) {
        throw std::invalid_argument("can't compare empty processing instruction");
    }

    if (!q->name) {
        throw std::invalid_argument("cannot compare empty processing instruction");
    }
